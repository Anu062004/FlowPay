...
function fmtProtocolLabel(value: string): string {
  return value
    ...
    .map((part) => {
      ...
      if (upper === "USDC" || upper === "USDT" || upper === "PT") {
        return upper;
      }
...
function normalizeRunResult(raw: InvestmentRunResult | Record<string, unknown>): InvestmentRunResult {
  const record = raw as Record<string, unknown>;
  const txHashesCandidate = record.txHashes ?? record.tx_hashes;
  const txHashes = Array.isArray(txHashesCandidate)
    ? txHashesCandidate.filter((value): value is string => typeof value === "string")
    : [];

  return {
    action: (typeof record.action === "string" ? record.action : "HOLD") as InvestmentRunResult["action"],
    confidence: typeof record.confidence === "number" ? record.confidence : 0,
    reasoning: typeof record.reasoning === "string" ? record.reasoning : "",
    invested_amount: typeof record.invested_amount === "number" ? record.invested_amount : 0,
    txHashes,
    allocation:
      record.allocation && typeof record.allocation === "object"
        ? (record.allocation as InvestmentRunResult["allocation"])
        : {},
    policy: record.policy as InvestmentRunResult["policy"]
  };
}
...
export default function InvestmentsPage() {
  ...
  const tradingAgentsHealthModel = useMemo(
    () => getHealthModel(tradingAgents?.health ?? null),
  ...
  const summary = useMemo(() => {
    ...
    const investmentPool = data?.summary?.investment_pool ? parseFloat(data.summary.investment_pool) : 0;
    return { totalDeployed, totalYield, activePositions, investmentPool };
  }, [data?.summary?.investment_pool, positions]);
  ...
  async function handleRunTradingAgents() {
    ...
    try {
      const normalized = normalizeRunResult(
        await runInvestmentRequest(activeCompanyId) as InvestmentRunResult | Record<string, unknown>
      );
      setRunResult(normalized);
      setRunMessage(
        normalized.txHashes.length > 0
          ? `TradingAgents returned ${normalized.action} and submitted ${normalized.txHashes.length} execution transaction${normalized.txHashes.length === 1 ? "" : "s"}.`
          : `TradingAgents returned ${normalized.action}. No execution transaction was submitted in this run.`
      );
    ...
    <div className="stack-xl">
      <div className="page-header-row">
        <div className="page-header">
          ...
          <p className="page-subtitle">
            TradingAgents-driven stablecoin treasury allocations with automated execution across supported DeFi venues
          </p>
        </div>
        <div className="row">
          <button
      ...
      {treasuryAddress ? (
          ...
          <div className="card-header">
            <div>
              ...
              <div className="card-subtitle">
                Approved investment allocations are executed from this company treasury wallet.
              </div>
            </div>
            ...
            <div className="grid-2" style={{ alignItems: "start" }}>
              <div className="stack" style={{ gap: 12 }}>
                ...
                <div className="alert alert-info">
                  ...
                  <span>
                    FlowPay validates policy first, then executes approved allocations against this treasury wallet.
                  </span>
                </div>
              </div>
              ...
              <div className="grid-2">
                <div className="metric-card">
                  ...
                  <div className="metric-card-change neutral">{treasuryTokenSymbol} currently available in treasury</div>
                </div>
                <div className="metric-card">
                  <div className="metric-card-label">Settlement Token</div>
                  <div className="metric-card-value font-num">{treasuryTokenSymbol}</div>
                  <div className="metric-card-change neutral">Primary treasury asset used for investment settlement</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="alert alert-warning">
          <span className="alert-icon"><Icon d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" size={16} /></span>
          <span>No treasury wallet is attached to the active company yet.</span>
        </div>
      )}
      ...
      <div className="card">
        <div className="card-header">
          <div>
            ...
            <div className="card-subtitle">
              Service health, executable protocols, and the latest recommendation captured for this company.
            </div>
          </div>
          <Badge
            ...
            <div className="stack" style={{ gap: 12 }}>
              <div className="grid-2">
                <div className="metric-card">
                  ...
                  <div className="metric-card-change neutral">Remote decision engine configured for this investment rail</div>
                </div>
                <div className="metric-card">
              ...
              {tradingAgents?.healthError ? (
                ...
                <div className="alert alert-info">
                  <span className="alert-icon"><Icon d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" size={16} /></span>
                  <span>TradingAgents health checks are passing. Timeout budget: {tradingAgents?.timeout_ms ?? 0} ms.</span>
                </div>
              )}
            </div>
                  ...
                  <div className="card-body">
                    <div className="empty-state" style={{ padding: "28px 0" }}>
                      ...
                      <div className="empty-state-desc">
                        Use the action above to run the investment engine and capture the first on-platform recommendation for this company.
                      </div>
                    </div>
                  </div>
                </div>
      ...
      <div className="grid-4">
        <div className="metric-card">
          <div className="metric-card-header">
            <div className="metric-card-label">Investment Pool</div>
          ...
          <div className="metric-card-value font-num">{fmtToken(summary.investmentPool, treasuryTokenSymbol)}</div>
          <div className="metric-card-change neutral">Treasury capital currently earmarked for investment automation</div>
        </div>
        ...
        <div className="metric-card">
          ...
          <div className="metric-card-change neutral">Capital deployed across supported protocols</div>
        </div>
      ...
      {loading ? (
        <div className="card">
          <div className="card-body">
            <div className="text-sm text-secondary">Loading investment data...</div>
          </div>
        </div>
      ) : (
        <>
                  ...
                  <tbody>
                    {positions.map((position) => (
                      <tr key={position.id}>
                        ...
                        <td>
                          <Badge variant={positionBadgeVariant(position.status)}>{position.status}</Badge>
                        </td>
